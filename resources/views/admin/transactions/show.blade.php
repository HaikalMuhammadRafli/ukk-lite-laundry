...
@section('content')
    <div class="container-fluid">
        <div class="admin-transactions-show-main" id="main">
            ...
            <div class="card p-3">
                ...
                @if ($transaction->payment_status != 'Pending')
                    @if ($transaction->status == 'New')
                        <a href="{{ route('transaction.status', $transaction->id) }}" class="btn btn-secondary mb-3">Process</a>
                    @elseif ($transaction->status == 'Processing')
                        <a href="{{ route('transaction.status', $transaction->id) }}" class="btn btn-secondary mb-3">Complete</a>
                    @elseif ($transaction->status == 'Completed')
                        <a href="{{ route('transaction.status', $transaction->id) }}" class="btn btn-secondary mb-3">Taken</a>
                    @endif
                    <button type="button" class="btn btn-primary" onclick="window.print()">Print</button>
                @else
                    <a href="{{ route('transaction.pay', $transaction->id) }}" class="btn btn-success">Pay Up Front</a>
                @endif
            </div>
        </div>
    </div>
